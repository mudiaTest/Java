...
class Queue {
	...
	public synchronized Object get() throws InterruptedException{
		while (head == null) //w pętli próbuje pobrać obiekt po 1,5 sek. Ta akcja blokuje wsztystkie bloki synchronized(this)
...
public class ParallelTest {
    ...
    //Volatile sprawia, że zmianna jest widoczna dla wszystkich wątków. Wpp wartość zmiennej nie zostanie odświerzona.
    //Volatile NIE ZAPEWNI locka na zmiennej, więc należy uzywać TYLKO jeśli jedno ze źródeł zapisuje a reszta czyta
...
//      ParallelCounter();
    ...
    public static void RunCallableDontStop() throws Exception{
        ExecutorService exec = Executors.newCachedThreadPool();       
        //Oczekiwanie na wynik pojedynczego wątku - program dalej działa i można kontrolować wątek
    ...
    public static void RunCallableWaitForFirst() throws Exception{
        ...
        //Future<Integer> results31 = invokeAny(tasks3);// odda tylko pierwszy wykonany task, a resztę zrobi cancel
    }
    ...
    //test na współdzielenie przy pomocy "volatile". Można też wykorzystać STATIC i FINALLY
    ...
    public static void ParrarelStream(){
        ...
        System.out.print("S:");
        String res1 = str1.chars().parallel().filter(s -> s > 0).mapToObj(s -> String.valueOf(((char)s))).peek(s -> System.out.print(" " + s)).collect(Collectors.joining(" "));        
        System.out.println("\nR: " + res1);
    }
    ...
    //wydajność: map can efficiently support a large number of concurrent readers and a certain number of concurrent writers
    ...
    //Stosować gdy mało wątkow wypeminia, ale wiele musi opracowywać (opracowanie jest bardziej pracochłonne niż wypełnianie)
    ...
    static Callable<Integer> taskConsumer = () -> { 
            ...
            try{
                do{
                    ...
                    //q.poll(100, TimeUnit.MILLISECONDS); //próba odczytania będzie trwała przez 100 ms, po tym czasie jeśli kolujka wciąz będzzie pusta to odda null
    ...
    //ConcurrentSkipListMap - używać jeśli zachodzi potrzeba na przechodzenie po posortowanych kluczach
    ...
    public static void AtomicTest(){
        ...
        //Gdy wiele wątków jednocześnie chce dostępu do generatora mogą pojawić się problemy z wydajnością
        //Nalezy zastosować
        ...
        //Problemem jest, że increment nie odda wartości, więc średnio to nadaje się do generowania id, bo te trzeba odczytać.
        //Można za to wykorzystać to jako sumator dla wielu równolegle działających wątków.
        
        //LongAccumulator - to generalizacja LongAdder. Do konstruktora sam podajesz sposób zwiększania wartości i wartość startową
        ...
        //Poniższe computeIfAbsent pozwala na zawsze uzyskanie wartość - oddaje gry w mapie nie ma key
    ...
    public static void ThreadSimple() throws InterruptedException{
		...
		thread.join(1000);  //Rozpoczyna pracę i czeka na jej zakończenie 1sek. Jeśli się nie doczeka, to zabija wątek
    ...
    public static void ThreadMisc() {
    	...
    	//thread.setDaemon(true): When only daemon threads remain, the virtual machine exits
    }
    
    //UI vs długo trwające zadania
    public static void ThreadLong() {
    	//brak jakiegokolwiek opisu aczkowiek wynika, że należy petrzeć na konkretny UI (Swing, android, java FX) 
    }
	...
	public static void Chain() throws InterruptedException{
		...
		//ver2
		task1(3).thenApply((i) -> task22(i)).thenAccept((s) -> {try {
																	task3(s.get());
																} catch (InterruptedException e) {
																	// TODO Auto-generated catch block
																	e.printStackTrace();
