...
public class JacksonTest {
  ...
  public void testBasic() throws Exception {
    ...
    // objectMapper.configure(Feature / MapperFeature. pozwala na zmian� ustawi� mappera, np ignorowanie adnotacji
    objectMapper.configure(Feature.AUTO_CLOSE_TARGET, false); // Zapobiega zamkni�ciu targetu - poni�ej System.out
    ...
    // Stringa J mo�na te� useska pisz�c go bezpo�rednio.
    ...
    // Zr�d�a: plik, input stream, URL, string 
...
//    objectMapper.readValue(new URL("file:src/test/resources/mo.json"), MyObject1.class);
  ...
  {
    ObjectMapper objectMapper = new ObjectMapper();
    // Nezb�dne do dzioa�ania @JsonRootName
    objectMapper.enable(SerializationFeature.WRAP_ROOT_VALUE);
    WrapObj w = new WrapObj();
    String stW = objectMapper.writeValueAsString(w);
    // Deserializacjia nie dzia�a bezpo�rednio bo obiektu WrapObj. 
  ...
  {
    ObjectMapper objectMapper = new ObjectMapper();
    // Niezb�dne dla parsowania dat - przynajmniej tych z Java 8
    objectMapper.findAndRegisterModules();
    FormatObject o = new FormatObject();
    o.dt = LocalDateTime.now();
    String s = objectMapper.writeValueAsString(o);
    // Dzia�a tylko do serializacji. Przy deserializacji b�d� b��dy
  ...
  public void viewTest() throws JsonProcessingException
  {
    ...
    // Deserializacj� wynikujemy bez modyfikatora, bo nadmiarowe pola b�d� null/domy�lne
