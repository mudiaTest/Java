...
//    return w1.getId();
	...
	public void testNoEntityManager() {
		...
		em.merge(p);
		// em.persist(p); //padnie z błędem "No EntityManager with actual transaction
		// available for current thread"
	}
	...
	public void insertByServiceNoTransactional() {
		Person p = new Person(1, "p1");
		ps.insertNoTransactionalPerson(p); // padnie z błędem "No EntityManager with actual transaction available for
		                                   // current thread"
	...
	// Ogólnie można mieszać @Transactional z TransactionTemplate pod warunkiem, że
	// PlatformTransactionManager będzie to samo
	// Nawet przymieszaniu propagacja bedzie działać tak, jakby było wykorzystanie
	...
	public void testTransactionTemplate2() {
		try {
			...
			tt.execute(status -> {
				...
				i = 0;
				// @Transactional(propagation = Propagation.REQUIRES_NEW) - zmiana BĘDZIE
				// widzoczna w DB managerze
				ps.insert2(new Person(1, "p3"));
				i = 0;
				// TransactionTemplate PROPAGATION_REQUIRES_NEW - zmiana BĘDZIE widzoczna w DB
				// managerze
				// Nowa transakcja powoduje zawieszenie aktualnej i powrów po zakończeniu nowej
				// ! Ważne, że wewnętrzny TransactionTemplate jest utworzony nad tym samym
				// PlatformTransactionManager
				// !! WAŻNE - ten nowy stan nie będzie widoczny dla zewnętrznej transakcji i na
				// odwrót !!
				// Wewnętrzna transakcja na stan z przed rozpoczęcia zewnętrznej!
				// Może się więc zdarzyć, że wewnęttrzna spowoduje błedy i rollbacj zewnętrznej
				// lub odczyt "złych" (nieintuicyjnych) danych.
