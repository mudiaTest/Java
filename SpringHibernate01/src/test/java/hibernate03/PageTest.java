...
//    t23.setStrVal("d");
  ...
  public void pageableTest() {
    try {
      ntw.inNewTrans(()->fillTest3());
      /*
       * Tworzenie obiektu pagable, kt�ry pozwloli na pobranie 1 strony w rozmiarze 2 element�w
       * Co sprowadza si� do "limit 2" 
       * "Strony" numerowane s� od 0
       */
      ...
      /*
       * Tworzenie obiektu pagable, kt�ry pozwloli na pobranie 5 strony w rozmiarze 2 element�w (9 i 10 element)
       * Co sprowadza si� do "limit x" dla (0,x) i "limit 2 offset 8" dla (4, 2)
       * "Strony" numerowane s� od 0 
       */
      //Pageable pageable = PageRequest.of(4, 2);
      Page<Integer> res = t1d.getPaged(pageable);
//      Zawarto�� wyniku to List<X>
  ...
  //Testowanie "Slice"
  //Zsadniczo dzia�a tak jak stronicowanie, ale zwraca inny obiekt, pozwalaj�cy na pobranie kolejnej partii wynik�w
  //Klasa Slice rozszerza klas� Page, wi�c getSliced jako argument  
  //@Test
  public void slicedTest() {
    try {
      ...
      ntw.inNewTrans(()->{
        ...
        List<Integer> lst = res.getContent();
        //Polecenie pobrania kolejnej porcji danych
  ...
  //Klasa Slice rozszerza klas� Page, wi�c getSliced jako argument  
  @Test
  public void slicedTestTest2() {
    try {
      ntw.inNewTrans(()->fillTest2());
      ntw.inNewTrans(()->{
      Pageable pageable = PageRequest.of(0, 2);
      Slice<Integer> res = t1d.getSlicedTest2(pageable);
      //Pobieranie wyniku zapytania
      List<Integer> lst = res.getContent();
      //Polecenie pobrania kolejnej porcji danych
      res = t1d.getSlicedTest2(res.nextPageable());
      lst = res.getContent();
      res = t1d.getSlicedTest2(res.nextPageable());
      lst = res.getContent();
      res = t1d.getSlicedTest2(pageable);
      lst = res.getContent();      
      int t = 0;
      });
    }
  ...
  //Testowanie "Page" dla customJOIN FETCH
  //!!! Pobierany jest ca�y wynik (bez ogranicze�) i dodatkowo wykonywany SELECT zliczaj�cy 
  //ilo�� wynik�w. Pagowanie wykonywane jest ju� na poziomie aplikacji, czyli niewiele daje.
  //@Test
